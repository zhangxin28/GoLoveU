...
// DefaultAvatar reprents the default avatar
...
// Generate performs to generate avatar bytes
func Generate(uid int64) ([]byte, error) {
	buf := new(bytes.Buffer)
	img := GenerateAvatar(uid)
...
// GenerateAvatar returns the avatar image for the uid
func GenerateAvatar(uid int64) image.Image {
	return identiconIns.Make([]byte(strconv.FormatInt(uid, 10)))
}
