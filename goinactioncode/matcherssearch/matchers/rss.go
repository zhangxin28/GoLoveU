...
func init() {
	...
	Register("rss", matcher)
}

// Search looks at the document for the specified search term.
func (m rssMatcher) Search(feed *data.Feed, searchTerm string) ([]*data.Result, error) {
...
func (m rssMatcher) retrieve(feed *data.Feed) (*rssDocument, error) {
	if feed.URI == "" {
